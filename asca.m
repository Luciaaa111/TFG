...
%
% reps = 4;
% vars = 400; lo que medimos en cada observación
% levels = {[1,2,3,4],[1,2,3]};
%
% F = create_design(levels,reps);
%
...
%Aplicamos PCA sobre los efectos extraídos
%del modelo GLM


%recorremos todos los factores 
for factor = 1 : ascao.n_factors
    
    %xf es la matriz de efectos del factor actual
    %Es decir: si tenemos X(con 8 observaciones 
    %y 400 variables) xf es una matriz 8x400 y representa
    %solo la parte de los datos explicada por ese factor
    xf = ascao.factors{factor}.matrix;
    %A continuación,aplicamos PCA
    %p son los loadings: direcciones (combinaciones de las variables originales
    % que capturan mayor variación debida al factor
    %Es decir, resumimos esas 400 varibales en menos que represetan lo que
    %hace el factor.
    %Resumen: p son los autovectores, cada fila de p
    %nos dice cuanto contribuye cada variable a cada PC

    %PCA sobre la matriz xf y retén tantos componentes principales
    % como el rango de xf permita
    p = pca_pp(xf,1:rank(xf));
    %Varianza de cada variable 
    ascao.factors{factor}.var = trace(xf'*xf);
    %Indicamos que hemos usado 2 factores
    ascao.factors{factor}.lvs = 1:size(p,2);
    ascao.factors{factor}.loads = p;
    %Proyecta el efecto xf en las direcciones principales p.
    %Nos dice cómo se sitúa cada observación según 
    % el efecto del factor en esas nuevas direcciones principales.
    ascao.factors{factor}.scores = xf*p;
    %Scores teniendo en cuenta el error
